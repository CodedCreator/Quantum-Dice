...
void BNO055_Motion::update() {
  ...
  if (_tumbleReferenceSet && deltaTime > 0.0 && deltaTime < 1.0) {
...
void BNO055_Motion::updateUpVector(float deltaTime) {
  // Calculate rotation angles from gyroscope readings
  // Gyro is in rad/s, multiply by deltaTime to get rotation angle in radians
  float xRot = _gyro.x() * deltaTime;
  float yRot = _gyro.y() * deltaTime;
  float zRot = _gyro.z() * deltaTime;

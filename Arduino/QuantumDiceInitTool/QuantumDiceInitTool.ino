...
const DiceConfig defaultConfig = {
  ...
  .tumbleConstant    = 0.2,
  .deepSleepTimeout  = 300'000  // 5 minutes
};
...
void loop() {
    if (Serial.available() > 0) {
        ...
        switch (input) {
            case '1': getMacAddress(); break;

            case '3': calibrateBNO055(); break;

            case '5': testBNO055(); break;

            case '2': configureEEPROMSettings(); break;

            case '4': clearEEPROM(); break;

            case 'M': displayMainMenu(); break;
...
void printEvent(sensors_event_t *event) {
    double x = -1'000'000, y = -1'000'000, z = -1'000'000, mag = 0;

...
const StateMachine::StateFunctions StateMachine::stateFunctions[] = {
    ...
    {&StateMachine::enterINITENTANGLED_AB1,
     &StateMachine::whileINITENTANGLED_AB1},
    ...
    {&StateMachine::enterINITENTANGLED_AB2,
     &StateMachine::whileINITENTANGLED_AB2},
    {&StateMachine::enterINITSINGLE_AFTER_ENT,
     &StateMachine::whileINITSINGLE_AFTER_ENT}};

// Get MAC address for a specific role - use config values
uint8_t* getMacForRole(Roles role) {
    switch (role) {
        case Roles::ROLE_A:
            return currentConfig.deviceA_mac;
        case Roles::ROLE_B1:
            return currentConfig.deviceB1_mac;
        case Roles::ROLE_B2:
            return currentConfig.deviceB2_mac;
        default:
            return nullptr;
    }
}
...
void printRole(Roles role) {
    ...
    switch (role) {
        case Roles::ROLE_A:
            Serial.println("ROLE_A");
            break;
        case Roles::ROLE_B1:
            Serial.println("ROLE_B1");
            break;
        case Roles::ROLE_B2:
            Serial.println("ROLE_B2");
            break;
        default:
            Serial.println("NONE");
            break;
...
void printStateName(const char* objectName, State state) {
    static State previousState =
        State::IDLE;  // Local static variable to retain
...
void StateMachine::sendMeasurements(Roles targetRole, State state,
                                    DiceStates diceState,
...
const StateTransition StateMachine::stateTransitions[] = {
    ...
    {State::WAITFORTHROW, Trigger::entangleStopReceived,
...
void StateMachine::enterINITENTANGLED_AB1() {
    ...
    if (roleSelf == Roles::ROLE_B1) {
        ...
        delay(100);
        sendMeasurements(roleB2, stateSelf, diceStateSelf, diceNumberSelf,
    ...
    if (roleSelf == Roles::ROLE_A) {
        ...
        sendMeasurements(roleB1, stateSelf, diceStateSelf, diceNumberSelf,
                         upSideSelf, measureAxisSelf);
    } else if (roleSelf == Roles::ROLE_B1) {
        debugln("B1 send measurement to A");
        sendMeasurements(roleA, stateSelf, diceStateSelf, diceNumberSelf,
                         upSideSelf, measureAxisSelf);
    }
...
void StateMachine::enterWAITFORTHROW() {
    ...
    if (diceStateSelf !=
        DiceStates::MEASURED) {  // no refresh in measured state, because this
...
void StateMachine::whileWAITFORTHROW() {
    ...
    if (checkMinimumVoltage()) {
        ...
        changeState(Trigger::buttonPressed);
                   ...
                   currentConfig
                       .tumbleConstant)) {  // Use tumble constant from config
    ...
    if ((diceStateSelf == DiceStates::ENTANGLED_AB1 ||
        ...
        (millis() - stateEntryTime >
         MAXENTANGLEDWAITTIME)) {  // return to initSingle state
        changeState(Trigger::timed);
    }
...
void StateMachine::whileTHROWING() {
    if (checkMinimumVoltage()) {
        changeState(Trigger::lowbattery);
               ...
               (withinBounds(abs(_imuSensor->getXGravity()), LOWERBOUND,
                             UPPERBOUND) ||
                withinBounds(abs(_imuSensor->getYGravity()), LOWERBOUND,
                             UPPERBOUND) ||
                withinBounds(abs(_imuSensor->getZGravity()), LOWERBOUND,
                             UPPERBOUND))) {
...
void StateMachine::enterINITMEASURED() {
    ...
    if (currentConfig.isNano) {
        ...
        if (withinBounds(abs(_imuSensor->getXGravity()), LOWERBOUND,
                         UPPERBOUND)) {
        ...
        } else if (withinBounds(abs(_imuSensor->getYGravity()), LOWERBOUND,
                                UPPERBOUND)) {
        ...
        } else if (withinBounds(abs(_imuSensor->getZGravity()), LOWERBOUND,
                                UPPERBOUND)) {
    ...
    switch (diceStateSelf) {
        ...
        case DiceStates::MEASURED:  // 2 options: same measureAxisSelf or
            ...
            if (measureAxisSelf !=
                prevMeasureAxisSelf) {  // different, generate random upNumber
        ...
        case DiceStates::UN_ENTANGLED_AB1: {
            ...
            if (currentConfig.alwaysSeven) {
                ...
                diceNumberSelf = (measureAxisSelf == measureAxisSister)
                                     ? selectOppositeOneToSix(diceNumberSister)
                                     : selectOneToSix();
                debugln((measureAxisSelf == measureAxisSister)
                            ? "sister ready. same axis"
                            : "different axis");
            }
            // Send measurements to the opponent dice
            Roles targetRole = (roleSelf == Roles::ROLE_A) ? roleB1 : roleA;
            sendMeasurements(targetRole, stateSelf, DiceStates::MEASURED,
                             diceNumberSelf, upSideSelf, measureAxisSelf);
            break;
        }
        ...
        case DiceStates::MEASURED_AFTER_ENT:  // 2 options: no sister diceNumber
            ...
            if (diceNumberSister ==
                DiceNumbers::NONE) {  // different, generate random upNumber
    ...
    prevDiceStateSelf = diceStateSelf;  // store for the future
    diceStateSelf =
        DiceStates::MEASURED;  // here the final diceState is set to measured
...
void StateMachine::whileLOWBATTERY() { voltageIndicator(XX); };

...
void StateMachine::enterINITENTANGLED_AB1()
{
  ...
  if (prevDiceStateSelf == DiceStates::ENTANGLED_AB2)
  { // only for A
...
void StateMachine::enterINITENTANGLED_AB2()
{
  ...
  // now kill the other entanglement
  if (prevDiceStateSelf == DiceStates::ENTANGLED_AB1)
  { // only for A
    // and stop the entanglement AB2 by A
    if (roleSelf == roleA)
    {
...
void StateMachine::enterWAITFORTHROW()
{
  ...
  if (diceStateSelf != DiceStates::MEASURED)
  { // no refresh in measured state, because this is done in INITMEASURED
    refreshScreens();
  }
  sendWatchDog();
};
...
void StateMachine::whileWAITFORTHROW()
{
  ...
  else if (entangleStopRcv)
  { // quit the entanglement
  ...
  if (roleSelf == Roles::ROLE_A)
  {
    ...
    if (diceStateSelf != DiceStates::ENTANGLED_AB2)
    { // SINGLE or ENTANGLED_AB1
      // Broadcast pairing request periodically
  ...
  if ((diceStateSelf == DiceStates::ENTANGLED_AB1 || diceStateSelf == DiceStates::ENTANGLED_AB2) && (millis() - stateEntryTime > MAXENTANGLEDWAITTIME))
  { // return to initSingle state
    changeState(Trigger::timed);
  }
  /*
...
*/
  ...
  case DiceStates::MEASURED: // 2 options: same measureAxisSelf or different measureAxis
    ...
    if (measureAxisSelf != prevMeasureAxisSelf)
    { // different, generate random upNumber
  ...
  case DiceStates::MEASURED_AFTER_ENT: // 2 options: no sister diceNumber or with diceNumber
    ...
    if (diceNumberSister == DiceNumbers::NONE)
    { // different, generate random upNumber

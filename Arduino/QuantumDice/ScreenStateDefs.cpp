...
ScreenStates x0ReqScreenState, x1ReqScreenState, y0ReqScreenState,
...
void printDiceStateName(const char* objectName, DiceStates diceState) {
    static DiceStates previousDiceState =
        DiceStates::NONE;  // Local static variable to retain its value between
                           // function calls
                           /*
   DiceStates::SINGLE
...
bool findValues(State state, DiceStates diceState, DiceNumbers diceNumber,
                UpSide upSide, ScreenStates& x0ScreenState,
    ...
    for (const auto& entry : truthTable) {
        if (entry.state == stateSelf &&
            (entry.diceState == diceStateSelf ||
             entry.diceState == DiceStates::ANY) &&
            (entry.diceNumber == diceNumberSelf ||
             entry.diceNumber == DiceNumbers::ANY) &&
...
void checkAndCallFunctions(ScreenStates x0, ScreenStates x1, ScreenStates y0,
                           ScreenStates y1, ScreenStates z0, ScreenStates z1) {
...
void refreshScreens() {
    ...
    if (findValues(stateSelf, diceStateSelf, diceNumberSelf, upSideSelf,
                   x0ReqScreenState, x1ReqScreenState, y0ReqScreenState,
                   y1ReqScreenState, z0ReqScreenState, z1ReqScreenState)) {
        checkAndCallFunctions(x0ReqScreenState, x1ReqScreenState,
                              y0ReqScreenState, y1ReqScreenState,
...
MeasuredAxises getAxis(IMUSensor* imuSensor) {
    ...
    } else if (withinBounds(abs(imuSensor->getYGravity()), LOWERBOUND,
                            UPPERBOUND)) {
        return MeasuredAxises::YAXIS;
    } else if (withinBounds(abs(imuSensor->getZGravity()), LOWERBOUND,
                            UPPERBOUND)) {

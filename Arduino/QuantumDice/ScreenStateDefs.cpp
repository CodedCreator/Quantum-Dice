...
State stateSelf,
  stateSister; // state is used for TruthTable. Is copy of currenState.
...
DiceNumbers    diceNumberSelf,
  diceNumberSister; // diceNumberSister is number of sister, diceNumberSelf
                    // is own number
UpSide upSideSelf, prevUpSideSelf,
  upSideSister; // upSideSister is upside from the sister
...
void printDiceStateName(const char *objectName, DiceStates diceState) {
    static DiceStates previousDiceState = DiceStates::NONE; // Local static variable to retain its
...
*/
    if (diceState != previousDiceState) {
        ...
        debug(": ");
        switch (diceState) {
            case DiceStates::SINGLE:             debugln("DiceStates::SINGLE"); break;
            case DiceStates::ENTANGLED_AB1:      debugln("DiceStates::ENTANGLED_AB1"); break;
            case DiceStates::ENTANGLED_AB2:      debugln("DiceStates::ENTANGLED_AB2"); break;
            case DiceStates::UN_ENTANGLED_AB1:   debugln("DiceStates::UN_ENTANGLED_AB1"); break;
            case DiceStates::UN_ENTANGLED_AB2:   debugln("DiceStates::UN_ENTANGLED_AB2"); break;
            case DiceStates::MEASURED:           debugln("DiceStates::MEASURED"); break;
            case DiceStates::MEASURED_AFTER_ENT: debugln("DiceStates::MEASURED_AFTER_ENT"); break;
            case DiceStates::ALL:                debugln("DiceStates::ALL"); break;
            case DiceStates::NONE:               debugln("DiceStates::NONE"); break;
            case DiceStates::CLASSIC:            debugln("DiceStates::CLASSIC"); break;
        }
...
bool findValues(State state, DiceStates diceState, DiceNumbers diceNumber, UpSide upSide,
    ...
    for (const auto &entry : truthTable) {
        if (entry.state == stateSelf
            && (entry.diceState == diceStateSelf || entry.diceState == DiceStates::ANY)
            && (entry.diceNumber == diceNumberSelf || entry.diceNumber == DiceNumbers::ANY)
            && (entry.upSide == upSideSelf || entry.upSide == UpSide::ANY)) {
...
void callFunction(ScreenStates result, screenselections screens) {
    switch (result) {
        ...
        case ScreenStates::QRCODE:
            ...
            break;
        default: Serial.println("No specific function for state");
    }
}
...
void refreshScreens() {
    ...
    if (findValues(stateSelf, diceStateSelf, diceNumberSelf, upSideSelf, x0ReqScreenState,
                   x1ReqScreenState, y0ReqScreenState, y1ReqScreenState, z0ReqScreenState,
                   z1ReqScreenState)) {
...
DiceNumbers selectOneToSix() {
    ...
    switch (randomNumber) { // dit kan vast handiger
        case 1: return DiceNumbers::ONE; break;
        case 2: return DiceNumbers::TWO; break;
        case 3: return DiceNumbers::THREE; break;
        case 4: return DiceNumbers::FOUR; break;
...
DiceNumbers selectOppositeOneToSix(DiceNumbers diceNumberTop) {
    ...
    switch (diceNumberTop) { // select the opposite value
        case DiceNumbers::ONE:   return DiceNumbers::SIX; break;
        case DiceNumbers::TWO:   return DiceNumbers::FIVE; break;
        case DiceNumbers::THREE: return DiceNumbers::FOUR; break;
        case DiceNumbers::FOUR:  return DiceNumbers::THREE; break;

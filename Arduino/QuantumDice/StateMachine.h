...
#define MAXENTANGLEDWAITTIME \
    120000  // ms-en wait for throw in entangled wait, befor return to
            // intitSingle state
...
enum class Roles : uint8_t { ROLE_A, ROLE_B1, ROLE_B2, NONE };
...
class StateMachine {
   public:
    ...
    void setImuSensor(IMUSensor* imuSensor) { _imuSensor = imuSensor; }
   ...
   private:
    void determineRoles();
    void sendWatchDog();
    void sendMeasurements(Roles targetRole, State state, DiceStates diceState,
